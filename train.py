...
from constants import BATCH_SIZE, EPOCHS, DEFAULT_MODEL_PARAMS, SEED_VALUE, BATCH_PRINT_FREQ, GRAD_CLIP_VALUE, SAVED_MODEL_LOCATION
...
def train_and_evaluate_bert(train_dataloder, validation_dataloader, num_labels, model_size = "base", cased = False,\
    ...
    params = DEFAULT_MODEL_PARAMS, epochs = EPOCHS, save_model = True, save_location = SAVED_MODEL_LOCATION):
    ...
    for epoch in t:
        ...
        for step, batch in enumerate(train_dataloader):
            if step % BATCH_PRINT_FREQ == 0 and not step == 0:
                ...
                time_remaining = (len(train_dataloader) - step) * time_per_batch
                LOG.info(f"\nBatch {step} of {len(train_dataloader)}. Elapsed: {format_time(elapsed)}")
                LOG.info(f"Time left in this epoch: {format_time(time_remaining)}")
